...
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/comment/tpl/comment_list/1'){ ?><div class="message <?php echo escape($__Context->XE_VALIDATOR_MESSAGE_TYPE, false) ?>">
...
<form id="fo_list" action="./" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	<table id="commentListTable" class="x_table x_table-striped x_table-hover">
		<caption>
			<a href="<?php echo escape(getUrl('','module','admin','act','dispCommentAdminList','Y'), false) ?>"<?php if($__Context->search_keyword == ''){ ?> class="active"<?php } ?>><?php echo $__Context->lang->all;
if($__Context->search_keyword == ''){ ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('search_target','is_secret','search_keyword','N'), false) ?>"<?php if($__Context->search_target == 'is_secret' && $__Context->search_keyword == 'N'){ ?> class="active"<?php } ?>><?php echo escape($__Context->secret_name_list['N'], false);
if($__Context->search_target == 'is_secret' && $__Context->search_keyword == 'N'){ ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('search_target','is_secret','search_keyword','Y'), false) ?>"<?php if($__Context->search_target == 'is_secret' && $__Context->search_keyword == 'Y'){ ?> class="active"<?php } ?>><?php echo escape($__Context->secret_name_list['Y'], false);
if($__Context->search_target == 'is_secret' && $__Context->search_keyword == 'Y'){ ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('search_target','is_published','search_keyword','N'), false) ?>"<?php if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'N'){ ?> class="active"<?php } ?>><?php echo $__Context->lang->published_name_list['N'];
if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'N'){ ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('search_target','is_published','search_keyword','Y'), false) ?>"<?php if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'Y'){ ?> class="active"<?php } ?>><?php echo $__Context->lang->published_name_list['Y'];
if($__Context->search_target == 'is_published' && $__Context->search_keyword == 'Y'){ ?>(<?php echo escape(number_format($__Context->total_count), false) ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo escape(getUrl('', 'module', 'admin', 'act','dispCommentAdminDeclared'), false) ?>"><?php echo $__Context->lang->cmd_declared_list ?></a>
		...
		<tbody>
			<?php if($__Context->comment_list&&count($__Context->comment_list))foreach($__Context->comment_list as $__Context->no=>$__Context->val){ ?><tr>
				...
				<td>
					...
					<a href="<?php echo escape(getUrl('', 'mid', $__Context->module_list[$__Context->val->module_srl]->mid), false) ?>" target="_blank"><?php echo escape($__Context->module_list[$__Context->val->module_srl]->browser_title, false) ?></a> - 
					<?php } ?>
					<a href="<?php echo escape(getUrl('','document_srl',$__Context->val->document_srl), false) ?>#comment_<?php echo escape($__Context->val->comment_srl, false) ?>" target="_blank"><?php if(strlen($__Context->comment)){;
echo escape($__Context->comment, false);
}else{ ?><em><?php echo $__Context->lang->no_text_comment ?></em><?php } ?></a>
				</td>
				<td class="nowr"><a href="#popup_menu_area" class="member_<?php echo escape($__Context->val->member_srl, false) ?>"><?php echo escape($__Context->val->getNickName(), false) ?></a></td>
				<td class="nowr"><?php echo escape(number_format($__Context->val->get('voted_count')), false) ?>/<?php echo escape(number_format($__Context->val->get('blamed_count')), false) ?></td>
				<td class="nowr"><?php echo escape((zdate($__Context->val->regdate,"Y-m-d\nH:i:s")), false) ?></td>
				<td class="nowr"><a href="<?php echo escape(getUrl('search_target','ipaddress','search_keyword',$__Context->val->ipaddress), false) ?>"><?php echo escape($__Context->val->ipaddress, false) ?></a></td>
				<td class="nowr"><?php if($__Context->val->isSecret()){;
echo escape($__Context->secret_name_list['Y'], false);
}else{;
echo escape($__Context->secret_name_list['N'], false);
} ?></td>
...
echo $__Context->lang->published_name_list['N'];
				...
				<td><input type="checkbox" name="cart" value="<?php echo escape($__Context->val->comment_srl, false) ?>" /></td>
			</tr><?php } ?>
...
<?php Context::addJsFile("modules/comment/ruleset/deleteChecked.xml", FALSE, "", 0, "body", TRUE, "") ?><form  action="./" method="post" class="x_modal x" id="listManager"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" /><input type="hidden" name="ruleset" value="deleteChecked" />
	...
	<input type="hidden" name="search_target" value="<?php echo escape($__Context->search_target, false) ?>" />
